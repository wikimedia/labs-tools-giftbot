...
# Archiver replacement (dewiktionary)

# Copyright 2011, 2012, 2014 Giftpflanze

# autoarchiv-wikt.tcl is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by the
# Free Software Foundation, either version 3 of the License, or (at your
# option) any later version.

source api.tcl
source dewiktionary.tcl

set token [login [set wiki $dewiktionary]]

proc parsesection {section} {
	foreach ts [string map {MEZ CET MESZ CEST} [regexp -all -inline {[0-9]{2}:[0-9]{2}, [0-9]{1,2}\. (?:(?:Jan|Feb|Mär|Apr|Jun|Jul|Aug|Sep|Okt|Nov|Dez)\.*|Mai) [0-9]{4} \(MES{0,1}Z\)}\
	 $section]] {
			...
			dict with item2 {
				...
				if {$byteoffset ne {null} && $level == 2} {
					...
					set ret4 [parsesection [set section [string range $ret2 $byteoffset [set byteoffset2 [
						expr {([lindex $sections $offset2] ne {} && [dict get [lindex $sections $offset2] byteoffset] ne {null})?[dict get [lindex $sections $offset2]\
						 byteoffset]-1:{end}}
					]]]]]
